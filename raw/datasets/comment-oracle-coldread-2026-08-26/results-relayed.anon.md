# Results — relayed, not verbatim (identifiers anonymized)
...
> Anonymized copy of `results-relayed.md` for the public repository. `UserRecord`, `AccountSvc`,
> `ParentIdLookup`, `sp_RefreshTaskAuth`, `LoginCodes`, `AdminUserPageBase`, `ApprovedDomains` and
> `[KeyField(...)]` stand for the employer's real names; the original stays untracked on the capture
> machine.
...
| 7 | Can the existence check be deleted? | had the claim, flagged it unverifiable from the file | **UNKNOWN** |
...
null-hash behaviour, `LoginV2`'s blank-name redirect, and `ParentIdLookup`'s passthrough.
...
| Claim | Reader | Status |
|---|---|---|
| `UserRecord` keyed on `UserName` alone (no `AccountId`), so the existence check matches site-wide | both | **CONFIRMED** — `[KeyField("UserName")]` is the only key; `GetByKey` selects by key |
...
| `UserTypeID = 1`, `AccountDisabled = 0`, `EmailVerified = false` duplicate constructor defaults | A | **CONFIRMED** — set at the corresponding backing fields in `UserRecord()`, and `UserRecord(string)` chains `: this()`. Initially misjudged as unsupported because the first check grepped property names rather than backing fields |
...
| `ParentIdLookup` cannot replace the existence check — returns its input unchanged for an unknown account | both | **CONFIRMED** |
| `AdminUserPageBase` writes an unusable random password on passwordless admin add, where this method leaves `PasswordData` NULL | B | **CONFIRMED** — two live conventions for the same intent |
| `LoginCodes.NormalizeEmail` upper-cases its key while this method stores the email as typed | B | **CONFIRMED but narrow** — scoped to login-code and throttle keys, not the `UserRecord` lookup, which SQL collation handles |
...
| `sp_RefreshTaskAuth` may delete-then-insert without a transaction | A, B | **UNVERIFIABLE** — the proc is not in the repository |

## What neither reader could settle with full repository access

- Whether `sp_RefreshTaskAuth` behaves correctly for a universal (null-`AccountId`) group, or
...
- Whether `UserRecord.AccountId` is FK-constrained (no DDL in the repo).
...
3. **The partition is weaker than stated.** The approved-domains table is keyed on `id`, not on the
   domain name, so nothing prevents two rows claiming one domain; the by-domain lookup returns a
