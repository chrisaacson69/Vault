...
**⭐ A sibling in the *we need what we cannot complete* family (2026-09-09):** [The Slack in the Law](../morality/legal-theory/slack-in-the-law.md) takes the same shape down one level — from *can a constitution bind its own enforcer?* to *can a finite code decide every case?* **It cannot**, and the argument is **pigeonhole rather than Gödel**: a finite set of general rules over an unbounded case-space must group together cases that differ in morally relevant ways, so edge cases are arithmetically certain rather than evidence of careless drafting (the rigorous jurisprudential form is Hart's **open texture**). The institutional consequence is that every legal order needs an **override it cannot inspect** — which is what a jury is, and why its acquittals are unreviewable. That page's worry is that **automation deletes the override while leaving the incompleteness**, which is this page's enforcement-bypass problem arriving from the opposite direction.
