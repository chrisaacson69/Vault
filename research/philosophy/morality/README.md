---
status: "active — core framework established, open questions remain"
created: 2026-02-16
published: true
---
# Morality — First Principles Exploration
> Naturalistic moral framework built from bedrock concepts upward. Anchor page for Trunk 1.

**Links:** [The Four Trunks](../the-four-trunks.md), [Philosophy](../README.md), [Economics — Value and Profit](../../economics/value-and-profit.md), [Scope Confusion](./scope-confusion.md)

**Trunk:** [Trunk 1 — Normative Ontology](../the-four-trunks.md#trunk-1--normative-ontology). Working name: *constitutive realism*.

## The framework in three lines

- **Morality requires agents** — physical reality, consciousness, agency (the [is-ought bridge](./is-ought-and-performative-grounding.md) is performative, not logical).
- **Given agents, the norms are constitutive** — not arbitrary, entailed by the structure of agent-interaction. See [Constitutive/Elective Distinction](./constitutive-elective.md).
- **Norms are scope-bounded, not universal** — core norms are structurally required; marginal norms are elective. See [Scope Confusion](./scope-confusion.md).

## Three overlapping circles

- **Morality** — right and wrong at individual conscience
- **Ethics** — group norms
- **Law** — codified ethics with enforcement

These overlap, they don't nest. Confusing the circles produces most political disagreement — see [scope-confusion](./scope-confusion.md).

## Two layers of good and bad

Action (praxeology) and morality aren't identical. Praxeological success = "did you achieve your purpose." Moral evaluation = "does your purpose, when pursued, sustain or degrade the conditions under which agents can function." Successful theft is praxeologically good and constitutively bad. Full treatment: [Constitutive/Elective Distinction](./constitutive-elective.md).

## Sub-pages

- [Is-Ought and Performative Grounding](./is-ought-and-performative-grounding.md) — dissolves Hume's guillotine; escapes Agrippa's trilemma
- [Performative Grounding — Philosophical Lineage](./performative-grounding-lineage.md) — Aristotle through Hoppe; 2,400-year tradition
- [The Constitutive/Elective Distinction](./constitutive-elective.md) — replaces objective/subjective; morality mind-dependent AND constitutive; the vacuity test
- [Scope Confusion](./scope-confusion.md) — most political disagreements are scope errors; enforceability gradient
- [Deontology vs. Consequentialism](./deontology-consequentialism.md) — false dichotomy; deontology as cached consequentialism
- [Action/Inaction Asymmetry](./action-inaction-asymmetry.md) — grounds negative/positive rights
- [Debate FAQ](./debate-faq.md) — living document of objections and responses
- [Open Problems](./open-problems.md) — unresolved tensions and dissolved ones

## Applied: Legal Theory

Legal norms are constitutive of cooperative rule-systems — applied Trunk 1. See [Legal Theory](./legal-theory/README.md).

## Related trunks

- [Trunk 2 — Verification Epistemology](../epistemology/README.md) — how convergence tests constitutive claims
- [Stance-Independence Retreat](../epistemology/stance-independence-retreat.md) — counter-page naming evasion moves

## Tags

[philosophy](../../../tags/philosophy.md), [morality](../../../tags/morality.md), [natural-law](../../../tags/natural-law.md), [economics](../../../tags/economics.md)
