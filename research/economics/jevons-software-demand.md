...
---
# Jevons for Software
> Agentic coding collapses the price of producing code. The naive reading is that software spend falls. The **Jevons paradox** says the opposite is possible: when the price of a good falls, the quantity demanded can rise by *more* than the price fell, and total spend goes **up**. The entire argument reduces to one empirical parameter — the **price elasticity of demand for software** — and it says nothing on its own about whether *developers* capture the new spend. Software has climbed this ladder for seventy years and has never once produced the "less software" outcome; the caveats are about *when the ladder ends*, not whether the rung exists. The synthesis (§4a): the input that did not get cheap is not *running* verification but **gating the criteria** — and Jevons raises demand for exactly the thing the oracle pages say cannot be terminally supplied.

**Links:** [Economics](./README.md), [Arithmetic as a Purchase](../arithmetic-as-a-purchase.md) (**the vault's own sixty-year Jevons specimen — multiply got cheap and was consumed by orders of magnitude more**), [The Theory of the Firm](./theory-of-the-firm.md) (Coase — cheaper internal production moves the build/buy boundary inward), [The Cyborg Model](../cyborg-model.md) (which input *didn't* get cheap), [Oracles Are Objective Functions](../oracles-as-objective-functions.md) (**no terminal oracle — the floor is adopted, not discovered; the `add(a,b)` argument lives there**), [The Contract Model vs. the Substrate Model](../contract-vs-substrate.md) (adopt / hold / retire — the verification layer as the un-automated component), [Career Strategy](../../career/strategy.md) (where the conclusion lands), [Lemonade Stand Agents](./lemonade-stand-agents.md) (what agents still can't buy cheaply: unit-economic judgment)
...
### 4a. The un-cheapened input, named precisely: gating the criteria

"Verification" is too coarse. *Running* a check is automatable and is already cheap — an agent will execute a test suite, a bytecode diff, or a regen-guard all day. What did not get cheap is **choosing the check and deciding when it is enough.** The vault's oracle pages hold the reason: no terminal oracle exists — an oracle that could certify an answer could also produce it ([Oracles Are Objective Functions](../oracles-as-objective-functions.md)) — so every standard in use is *adopted*, not discovered, and the method is **adopt, hold, retire** ([the substrate model](../contract-vs-substrate.md)). Chris's pre-LLM form of the same point is the `add(a, b)` argument: the test domain is infinite or impractical for nearly every function, so "fully tested" is a fiction and every suite is a chosen good-enough. **The human value-add is gating the criteria** — where good-enough sits, for this artifact, and when a standard that used to discriminate has flattened ("all-pass is the signal").

Put the two halves together and the page's strongest claim appears. **Jevons says induced software volume raises the *demand* for gating** — each new artifact is both new software and a new thing the oracle chain must reach, which is why the burden of checking gets deeper as the tool bank grows, not shallower. **The oracle pages say the *supply* of gating is bounded by human altitude** — the grounding regress bottoms out at the person ([the grounding regress](../repairing-llm-code.md)), and headcount does not substitute for altitude. Rising demand against a supply that cannot scale is a **price-rise prediction** — Baumol, exactly, for the one component of the bundle that neither got cheap nor can be terminally solved. That is a stronger claim than either page makes alone, and it is the specific form of "the human holds the input that didn't get cheap."
