...
- **[Coordination Without a Coordinator](./coordination-without-a-coordinator.md)** — ***(economics)*** **a walled, self-perpetuating advantage can assemble itself out of individually rational decisions made on true information** — no agreement, no policy, no conspiracy, and therefore **no defendant to sue and no statute to repeal**. Tacit collusion is the narrow load-bearing case (antitrust needs an *agreement* and parallel conduct alone will not supply one); the general form drops that mechanism into a [positional good](./positional-goods.md) and builds a wall. ⭐ **GATTACA is the cleanest specimen and is routinely misread**: there is no law in the film: Vincent is denied *insurance* and *employment*, by actuaries and employers each acting alone on accurate information. ⭐⭐ **Which makes it the blind spot of any framework that defines coercion as state action** — *you cannot shrink your way out of an actuary* — with the mirror error noted for frameworks that see only private power. Carries its own deflaters and a **discriminating test: would a defector profit, and does anyone defect?** Anything called a citadel without that test is a vibe.
...
- [Jevons for Software](./jevons-software-demand.md) — ***(thesis)*** cheaper code → **more** software spend *if* demand is elastic (|ε| > 1); the 70-year base rate says yes, but **+spend ≠ +coders** (ATM-tellers vs. power-loom-weavers) and **elasticity is a phase**, not a constant. Only *generation* got cheap — the bottleneck migrates to **gating the criteria**, the input no oracle can terminally supply (Baumol)
...
### Theory
- [The Theory of the Firm and the Optimal Scale of Collectives](./theory-of-the-firm.md) — Coase's firm boundary (internal vs market transaction cost) generalized to every collective: firm → nation (Alesina-Spolaore) → empire. Optimal scale, not maximal; the inward/outward pressures are the doctrine-of-the-mean basin (symmetry-breaking); validated by christian-advantage's decentralization verdict. Pre-political: no scale is terminal, subsidiarity, the optimum moves with transaction-cost technology ⭐ **What large firms actually lack is *bids*, not prices** (2026-09-23) — they have cost accounting to the penny; what they cannot get is the counterfactual a rival would have bid, and *"could it be made or bought cheaper"* **is** Coase's make-or-buy margin. Profitability is a whole-firm signal and does not decompose, so the failure is quiet rather than chaotic — which is why firms manufacture bids (tendering, outsourcing pilots, benchmarking)
...
- [What Is Money?](./what-is-money.md) — *(seed)* store of value or medium of exchange? Chris leans store (*"we could all share the same dollar"*); the store-of-value function is the V term (Cambridge k = 1/V); his fixed-supply doubt is the free-banking side of an internal Austrian split.
