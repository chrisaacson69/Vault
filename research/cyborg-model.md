...
**Links:** [Eyes and Ears for LLMs](./eyes-and-ears-for-llms.md) — the audio translator: what lets the LLM *read* the score (three-tier stack, goals with oracles), [AI as a Cognitive Tool](./ai-as-cognitive-tool.md), [Economics](./economics/README.md), [Risk and Entrepreneurship](./economics/risk-and-entrepreneurship.md), [Praxis](./economics/praxis-agent-teams.md), [LLM Grounding Problem](./llm-grounding-problem.md), [Cognitive vs. Motor Skills](./cognitive-vs-motor.md), [Claude Opus 4.6 Research](./claude-opus-4-6.md), [Gaming](./gaming/README.md), [Measurement, Causality, and Free Will](./philosophy/metaphysics/measurement-causality.md), [Karpathy LLM Wiki Convergence](../notes/karpathy-llm-wiki-convergence.md), [The Three-Layer Method](./karpathy-three-layer-method.md), [Level 6 — Direct Execution](./level-6-direct-execution.md), [Career](../career/README.md), [Jevons for Software](./economics/jevons-software-demand.md) (why the human side of the split is holding the input that *didn't* get cheap)
...
- ~~Can the vault develop the equivalent of "intuition" — fast, cached retrieval paths that skip the full INDEX.md traversal? (Karpathy's "hot cache" was a primitive version.)~~ — **Answered (2026-08-26), by an existence proof rather than by us.** Nate B. Jones's "Ringer" is a local intermediary that sits between the client and the model provider and can *answer without a model call at all* — serving a previously accepted answer out of a store, or running a fixed local recipe — as well as selecting passages and enforcing hard request limits. That is precisely the "intuition" shape: a cached path that skips the traversal, placed **before** the request is assembled rather than inside it. Jones names the ceiling this clears, which is the part worth keeping: *a skill cannot make the call it is inside of any smaller* — by the time the model reads a skill, the preamble, standing instructions and tool definitions have already been sent. So the fast path has to live outside the call. Not adopted here (unverified third-party tooling), but the architecture question is settled: the layer exists, it goes in front, and it is a cache with an eviction policy. See [Building Swarms](../method/swarm-construction-rules.md) and [The Context Cache Hierarchy](../notes/context-cache-hierarchy.md).
