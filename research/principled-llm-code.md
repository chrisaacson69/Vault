...
**Links:** [AI History — A Personal Arc](./ai-history-personal.md), [The Context Cache Hierarchy](../notes/context-cache-hierarchy.md), [Claude Code Skill Engineering](../notes/claude-code-skill-engineering.md), [LLM Grounding Problem](./llm-grounding-problem.md), [Planner-LM Composites](./planner-lm-composites.md), [Conservation of Complexity](../notes/conservation-of-complexity.md), [Repairing LLM Code — The Two Oracles](./repairing-llm-code.md) — the slop has two axes: correctness (groundable) and readability (the choice function over equivalent renderings), [Comments and the Distance to an Oracle](./comment-oracle-distance.md) — mechanical-vs-analytical applied to comments — type them and eviction becomes deterministic, [The Contract Model vs. the Substrate Model](./contract-vs-substrate.md) — the field report: median-per-session slop as an auditor finds it, and why the contract that catches it doesn't compound
