...
## The pre-LLM proof: `add(a, b)` and the fiction of "fully tested"

The claim *the floor is adopted, not discovered* is older than any of the agent specimens on this page, and Chris had the argument before the vault existed. Provenance is his recollection of a discussion with Microsoft evangelists "a few years back" (undated, no transcript): **fully test `add(a, b)`.** Every value of `a` and `b`? Nulls? Overflow at every width? Signed and unsigned? The point was not that testing is hard; it was that **the domain is infinite, or so large that exhaustive coverage is impractical, for essentially every function anyone writes** — so "fully tested" is a fiction, and every real test suite is a *chosen* standard that someone decided was good enough. TDD does not escape this; it just moves the choice earlier and hides it inside the phrase "write the test first."

That is the same object as every oracle on this page. A test suite **orders** candidates (this build is closer to the spec than that one); it never **certifies** one, because certification would require the coverage no one has. The difference between "the tests pass" and "the code is correct" is exactly the difference between an objective evaluated and an optimum located. And it names the human's irreducible job more precisely than "judgment" or "taste" does: **the value-add is gating the criteria** — deciding *where* good-enough sits for *this* function, in *this* context, and knowing when a standard that used to discriminate has flattened. Running the check is automatable and is getting cheap. Choosing the check — and retiring it — is the input that is not, which is the [Jevons for Software](./economics/jevons-software-demand.md) §4 conclusion arriving from the other direction.
