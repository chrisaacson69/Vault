...
**Links:** [The LLM Grounding Problem](./llm-grounding-problem.md) — the parent: LLMs can't weight grounded truth over fluent text, [Practicality vs. Precision — Principled LLM Code](./principled-llm-code.md) — slop is a persistence problem, the economics just tipped, [Transpilation as a Grounding Strategy](./transpilation-as-grounding.md) — weak formal languages grounded by deterministic transpile, [The Anchor Method](../method/anchor-method.md) — the gate that *proves grounding*, [H-Neurons](./h-neurons.md) — hallucination as over-compliance; the coda argues confident-wrong-path is the same disease, [Nobunaga Decompiler](../projects/game-annotation/nes/na1/README.md) — the live case study, [Lowering Atlas](../projects/game-annotation/nes/na1/lowering-atlas/README.md) — sourcing the inverse-lowering table forward, [pygone](../projects/pygone/README.md) — the *inverse* specimen (readable Python → 4 KB blob): the floor-vs-ceiling complement to the decompiler's bytecode→readable-C direction, [The Three-Layer Method](./karpathy-three-layer-method.md) — the two-oracle / independent-critic idea as the practical "Verifier" layer, [Comments and the Distance to an Oracle](./comment-oracle-distance.md) — the child: the readability axis applied to prose-in-code, and the cost term the transfer test was missing, [The Contract Model vs. the Substrate Model](./contract-vs-substrate.md) — the child: *tests written only to pass* is oracle collapse in the field, and the artifact/reader split that makes an LLM verifier legitimate, [Oracles Are Objective Functions](./oracles-as-objective-functions.md) — the child: *oracle-discovery* and *objective specification* are one problem, and greedy decoding as a no-restart hill-climb is the mechanism behind self-sycophancy
