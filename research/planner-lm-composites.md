...
**Links:** [LLM Agents Across Strategic Games](./gaming/llm-agents-across-games.md), [Gunboat Diplomacy and Diplodocus](./gaming/gunboat-diplomacy-diplodocus.md), [CaptainMeme vs. 6 Cicero (Press Diplomacy)](./gaming/cicero-press-diplomacy-captain-meme.md), [The Multiplayer Coalition Problem](./gaming/multiplayer-coalition-problem.md), [LLMs as Praxeological Actors](./economics/llm-praxeology.md), [The LLM Grounding Problem](./llm-grounding-problem.md), [The Cyborg Model](./cyborg-model.md), [Level 6 — Direct Execution](./level-6-direct-execution.md), [Monopoly Project](../projects/monopoly/README.md), [Diplomacy: 7 AI Models](./gaming/diplomacy-ai-analysis.md), [Energy-Based Models](./energy-based-models.md) — names the middle layer concretely; Aleph/Lean is the existence proof, LeCun/JEPA is the academic convergence, [Game Annotation Series](../projects/game-annotation/README.md) — LLM-interpretation as another data point: structure-only analysis recovers shape, chip docs and cumulative chapter knowledge unlock the rest
...
- **Dedicated reasoning engine.** State model, value model, planner, verification. Deterministic where possible, learned where necessary. This is the Cicero/Diplodocus/Monopoly planner component, generalized to whatever domain the system operates in. The [energy-based-model lineage](./energy-based-models.md) (Hopfield → Boltzmann → JEPA → Aleph/Kona) is the strongest named candidate for this slot in domains where the problem decomposes into "score whether a whole state fits the constraints" — proofs, design, scheduling, verification.
...
**Sudoku as the cleanest demonstration.** Logical Intelligence's Kona solves a Sudoku in 0.4 seconds via constraint-satisfaction descent. Frontier LLMs given the same puzzle "solve" it only by writing and executing a Python brute-force solver — when code execution is disabled, they fail. The rules of Sudoku fit in one sentence, so the failure mode is unambiguous: tool-use is *outsourcing* the constraint check, not performing it. The LLM that builds a custom solver-per-problem is doing on-demand tool construction; the EBM-plus-verifier pipeline is using a general-purpose validation layer. The former scales linearly with tool-quality risk; the latter amortizes verification across problem classes. See [Energy-Based Models](./energy-based-models.md#general-purpose-validation-vs-on-demand-tool-building) for the full distinction.

**Aleph as the existence proof for the middle layer.** Logical Intelligence's Aleph produced Lean-checked proofs for 668/672 PutnamBench problems (99.4%) and 100% on Verina. This is the three-layer stack — LM proposer + EBM-class reasoner + formal verifier — operating publicly with benchmarks today, in the cleanest domain (formal math) where the verifier (Lean) is mature. The architectural form the vault converged on from game-AI and biology is now demonstrated to work in a non-game domain with audit-grade output.
...
- **[Energy-based models](./energy-based-models.md) are the strongest named candidate for the middle layer** in constraint-heavy domains. The three-layer stack (LM / EBM / formal verifier) has a public demonstration in Aleph + Lean at PutnamBench scale. LeCun's JEPA program is the academic convergence onto the same architecture from the Meta AI lineage — three independent convergences (vault, Karpathy, LeCun) onto the same skeleton is strong evidence about the shape of the problem.
- **Integration of components is easier than it looks.** Each interface in the LM ↔ reasoner ↔ verifier stack is narrow and well-typed (structured problem in, candidate state out, yes/no plus reason back). MCP and tool-use are the industry generalization. The intuition that combining a language model, a constraint engine, and a verifier is a deep architectural research problem is wrong; the form is right and the construction is mostly plumbing.
