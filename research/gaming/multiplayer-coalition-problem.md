...
- **Complete interference** (Risk, Diplomacy): the dogpile can fully neutralize a leader's progress. High-level play treats hidden-random as worst-case-assumed (Risk: "3+ cards AS-IF they already have the turn-in armies"), which collapses the information asymmetry and restores the perfect-info dogpile. Such games **terminate only via out-of-game persuasion** — convincing opponents to defect from the optimal coalition. Against a perfect AI that doesn't believe out-of-game promises, *neither Risk nor Diplomacy would ever end*. This is the structural reason Cicero needed both a planner AND a language model — the language model handles the persuasion the planner can't fake. ([[cicero-press-diplomacy-captain-meme]] and [[gunboat-diplomacy-diplodocus]] bracket this result: Cicero needs the language layer; Diplodocus pure-planner wins only in Gunboat where persuasion isn't allowed.)

- **Incomplete interference** (Catan, Poker, Monopoly): the dogpile can throttle progress but not stop it. A channel of advancement continues regardless — Catan's production-roll, Poker's forced bet-stream, Monopoly's mandatory dice. Such games **terminate under perfect AI** because progress accumulates whether opponents like it or not. Hidden-random adds a timing layer (decides *which turn* it ends) but is not the structural termination mechanism — that's the un-suppressible progress stream.
...
See [Catan 47k empirical — N≥3 termination](./catan-47k-empirical.md#n3-termination--interference-completeness-is-the-real-mechanism) for the full treatment with the game-class comparison table.
