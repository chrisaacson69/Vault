...
**Companion data:** [Catan — 47,000 Games of Empirical Findings](./catan-47k-empirical.md) — Ioana Roman's quantitative analysis of 47k recorded Catan games gives the *ground truth* this phase needs. The Catan-LLM phase documents what bad Catan play looks like (action bias, no mechanical model, rhetorical contagion); the 47k-games page documents what *good* Catan play looks like (city engine over road engine, trajectory-trades / winners overpay for position, late-game Monopoly timing). The two pages bracket the problem: failure modes on one side, structurally correct moves on the other, and every LLM in this phase is missing the same three structurally correct moves.
