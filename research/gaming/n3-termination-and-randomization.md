...
**Links:** [The Multiplayer Coalition Problem](./multiplayer-coalition-problem.md) (the gang-up equilibrium this terminates), [Risk — The Attrition Constant](./risk-attrition-odds.md) (**a second stabilizer in the same slot** — Risk's exact combat math shows the brake is not randomness *per se* but the **cost of a won battle**: attacking is tactically free of deterrence, yet the winner retains only ~15% of its force, so aggression is punished by the bystander rather than by the defender. Design lesson in the same family: omit tactical deterrence, recover stability from N), [Bilateral Trade Valuation](./bilateral-trade-valuation.md), [Game Theory as Normative, Not Descriptive](../../notes/game-theory-as-normative-not-descriptive.md), [Battleship — 30 Billion Boards](./battleship-board-analysis.md) (the **N=2 contrast case** — zero-sum, so a clean computable minimax equilibrium *does* exist, exactly the regime that breaks once N≥3)
