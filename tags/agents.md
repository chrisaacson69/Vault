...
- [Oracles Are Objective Functions](../research/oracles-as-objective-functions.md) — an agent driving at an objective fails in exactly two places: the objective is wrong (lemonade stand) or it cannot be evaluated (Claudius); Goodhart applies because these are maximisers, not generators
