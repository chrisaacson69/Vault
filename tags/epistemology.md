---
published: true
layout: layouts/page.njk
title: "Tag: epistemology"
permalink: /tags/epistemology/
---
# epistemology
> Files exploring how we know what we know — foundations of knowledge, justification, and belief.

- [Epistemology — Parking Lot](../research/philosophy/epistemology/README.md)
- [Performative Grounding — Philosophical Lineage](../research/philosophy/morality/performative-grounding-lineage.md)
- [Is God the Necessary Precondition for Knowledge?](../research/debates/god-precondition-knowledge-nicks-allen.md)
- [The Fallacy Fallacy](../research/philosophy/tangents/the-fallacy-fallacy.md)
- [TAG Debunks Atheism (Nicks vs Peterson)](../research/debates/tag-debunks-atheism-nicks-peterson.md)
- [Andrew Wilson vs Objectivism: Stance Dependence](../research/debates/wilson-objectivism-stance-dependence.md)
- [Relational Objectivity](../research/philosophy/epistemology/relational-objectivity.md)
- [Value/Utility via Evolutionary Game Theory (Evo-Cap)](../research/evolutionary-capitalist/value-utility-evolutionary-game-theory.md)
- [Gauge Theory Applied to Economics (Weinstein x Murphy)](../research/debates/weinstein-murphy-gauge-theory-economics.md)
- [Transhumanism vs Christianity (Wilson x Istvan)](../research/debates/wilson-istvan-transhumanism-christianity.md)
- [HoP Ep 1: Introduction](../research/philosophy/tangents/history-of-philosophy/01-introduction.md)
- [HoP Ep 3: Heraclitus](../research/philosophy/tangents/history-of-philosophy/03-heraclitus.md)
- [HoP Ep 6: Pluralists & Atomists](../research/philosophy/tangents/history-of-philosophy/06-pluralists-atomists.md)
- [Would a Perfectly Logical Society Be Less Humane? (Noerr)](../research/debates/noerr-logic-feelings-humane.md)
- [The Birthmark and the Search for Truth](../research/philosophy/epistemology/the-birthmark.md)
- [Force Doctrine and Pragmatism (Wilson)](../research/debates/wilson-force-doctrine-pragmatism.md)
- [A History of Logic](../research/philosophy/logic-and-math/history-of-logic.md)
- [HoP Ep 7: Sophists](../research/philosophy/tangents/history-of-philosophy/07-sophists.md)
- [HoP Ep 8: Socrates](../research/philosophy/tangents/history-of-philosophy/08-socrates.md)
- [HoP Ep 9: Plato's Metaphysics](../research/philosophy/tangents/history-of-philosophy/09-plato-metaphysics.md)
- [HoP Ep 10: Plato's Epistemology](../research/philosophy/tangents/history-of-philosophy/10-plato-epistemology.md)
- [HoP Ep 11: Plato's Ethics](../research/philosophy/tangents/history-of-philosophy/11-plato-ethics.md)
- [Karpathy LLM Wiki — Independent Convergence](../notes/karpathy-llm-wiki-convergence.md)
- [Subjective vs Objective Morality (Noerr vs Terry X)](../research/debates/noerr-teryx-subjective-objective-morality.md)
- [The Constitutive/Elective Distinction](../research/philosophy/morality/constitutive-elective.md)
- [The Stance-Independence Retreat](../research/philosophy/epistemology/stance-independence-retreat.md)
- [Lance Bush Reviews Wilson on Triggernometry](../research/debates/lance-bush-review-wilson-triggernometry.md) — Kaplan/Shafer-Landau tree + Lance's 4-axis grid + vault's constitutive axis
- [Weaponized Nuance](../notes/weaponized-nuance.md) — "it's nuance, bro" as a rhetorical move; under-committed mirror of the directional-truth trap
- [The Four Trunks — The Structure of the Vault's Philosophy](../research/philosophy/the-four-trunks.md)
- [HoP Ep 13: Aristotle Intro](../research/philosophy/tangents/history-of-philosophy/13-aristotle-intro.md)
- [HoP Ep 14: Aristotle's Epistemology](../research/philosophy/tangents/history-of-philosophy/14-aristotle-epistemology.md)
- [HoP Ep 15: Aristotle — Father of Logic](../research/philosophy/tangents/history-of-philosophy/15-aristotle-logic.md)
- [Convergence Is a Process, Not a Destination](../notes/convergence-as-process.md) — extends the Weighting Problem to dynamic systems; no stance-independent optimum even given fixed values, because the PPF moves and time preferences differ
- [Structure vs Adaptability](../notes/structure-vs-adaptability.md) — the universalization-as-tell ("for all" / "always" / "never" reveals overshooting); explains why most political disagreements are meta-disagreements about which pole is currently underweighted
- [Capitalism vs Socialism: Which Is the Moral System? (1984)](../research/debates/1984-peikoff-ridpath-vs-vickers-caplan-capitalism-socialism.md) — Peikoff/Ridpath vs Vickers/Caplan; Objectivist foundationalism vs pragmatic democratic socialism
- [Trent Horn vs NotSoErudite — Christian Nationalism](../research/debates/trent-horn-vs-notsoerudite-christian-nationalism.md) — "religious view" relabeling as scope expansion; verification-layer angle neither party uses
- [Lance Bush vs Ryan Mullally — Realism vs Global Anti-Realism](../research/debates/lance-bush-mullally-metaethics.md) — Bush's global anti-realism (incl. logic/epistemics) + the unintelligibility thesis vs "external reasons"; "can you say what it means?" as a reusable scalpel
- [The Constitutive Middle — Two Dialectics](../research/philosophy/morality/constitutive-middle-dialectics.md) — the global anti-realist ladder turns on epistemic/logical norms being constitutive (Tier 1) — demonstrable in the act of denying them
- [Rights Don't Actually Exist, Only Force (Wilson vs 5)](../research/debates/wilson-rights-dont-exist-only-force.md) — the "can't taste it → doesn't exist" criterion proves too much: it deletes numbers, logic, and the wielder's own duties and God
- [Christians Have as Much Right to Rule as Secularists (Wilson vs 5)](../research/debates/wilson-christians-right-to-rule.md) — a five-opponent natural experiment: holding the operator fixed and varying preparedness isolates how much of an argument's "success" is the move vs. the opponent's gaps; the foundation fight confirmed as a tar pit
- [Do Atheists Benefit from Living in a Christian Society? (Wilson vs 5)](../research/debates/wilson-atheists-benefit-christian-society.md) — the "even corrupted it works" move buys unfalsifiability (no specified disconfirmer); outcomes claims have truth-values where preferences don't; the comparative-data test neither side runs is the real epistemic work
- [Force Doctrine — The Theory](../research/philosophy/morality/force-doctrine.md) — material ≠ real; the tangibility criterion is self-refuting (an insensible proposition); both a right and a phone are known only through representations — vividness of access ≠ reality of referent
- [Accounting Identities as Domain-Matching, Not Behavioral Models](../research/economics/accounting-identities-as-domain-matching.md) — Macroeconomic identities like GDP = C+I+G+X−M and MV = PT *look* like equations with independent terms that can be moved around to predict policy outcomes.
- [Aristotelian Logic](../research/philosophy/logic-and-math/aristotelian-logic.md) — The first formal system of reasoning — and the default assumption in almost every philosophy debate today.
- [Deontology vs. Consequentialism: The False Dichotomy](../research/philosophy/morality/deontology-consequentialism.md) — They're not competing theories.
- [Directional Truth and the Discrete Trap](../notes/directional-truth-and-the-discrete-trap.md) — We want discrete destinations ("no government") when reality only supports directional vectors ("less government").
- [Eastern Logical Traditions](../research/philosophy/logic-and-math/eastern-logic.md) — The convergence proof: independent civilizations, no contact, same structural insights — centuries before the West caught up.
- [Emergence, Convergence, and the Strange Loop](../research/philosophy/logic-and-math/emergence-and-convergence.md) — The vault's metaphysical position, stated explicitly: what logic is, why it works, why it doesn't need God, and what it can't prove about itself.
- [Game Theory as Normative, Not Descriptive](../notes/game-theory-as-normative-not-descriptive.md) — Game theory is a normative theory of optimal play given a stated game structure.
- [Gödel's Incompleteness Theorems](../research/philosophy/logic-and-math/godel-incompleteness.md) — No sufficiently powerful formal system can be both complete and consistent.
- [Intuitionistic Logic](../research/philosophy/logic-and-math/intuitionistic-logic.md) — What if truth requires proof? Drop the excluded middle and logic becomes constructive — and accidentally invents the theoretical foundation of programming.
- [Is Art Purely Objective?](../research/debates/art-objectivity-noerr-strange.md) — A 6-hour no-prep debate that's really about epistemology, value theory, Agrippa's trilemma, and the is-ought gap — wearing an art costume.
- [Lyn Alden on Long-Term Trade Deficit Unsustainability — A Vault Framework Engagement](../research/economics/lyn-alden-trade-deficit-analysis.md) — Lyn Alden's [trade deficit analysis](https://www.lynalden.com/trade-deficit/) makes a careful, multi-level argument for the unsustainability of long-term trade deficits — operat…
- [Metaphysics](../research/philosophy/metaphysics/README.md) — How real immaterial patterns arise from organized material substrate.
- [Morality — Open Problems](../research/philosophy/morality/open-problems.md) — Where the framework is incomplete or under pressure.
- [Non-Classical Logics](../research/philosophy/logic-and-math/non-classical-logics.md) — What happens when you challenge the other axioms? Contradictions that don't explode, truth that comes in degrees, and premises that must actually be relevant.
- [Quantum Logic](../research/philosophy/logic-and-math/quantum-logic.md) — The other non-classical logics were invented by philosophers.
- [Measuring Inflation — Why Disaggregation Doesn't Save You](../research/economics/measuring-inflation.md) — the cleanest economic instance of the Weighting Problem, plus the proof it is **scale-invariant**: disaggregate to the individual and aggregation across *goods* is still required, so the index-number problem reappears at n=1. You cannot disaggregate your way out of a subjective aggregation function.
- [The Productivity–Pay Gap](../research/economics/productivity-pay-gap.md) — Blair Fix's aggregation critique is the Weighting Problem in another costume: prices do the aggregating, so identical physical output yields different "real" productivity depending on which year's prices you hold constant. "Productivity" so measured is income relabelled.
- [Poverty in America Is a Sign of Exploitation (prep)](../research/debates/poverty-exploitation-prep.md) — what a "sign" must be to carry evidential weight: it has to vary with what it signals, and be a departure from a baseline. A near-invariant metric can't be diagnostic of anything.
- [The Coconut Island and the Manufactured Option Space](../research/debates/coconut-island-and-manufactured-option-space.md) — Two related debate moves used to force "exploitation under capitalism" or "structural conflict" conclusions.
- [The Fiscal Pressure Must Leak Somehow — All of the Above](../notes/fiscal-pressure-must-leak-somehow.md) — Sketch-tier musing.
- [The Formal Revolution (1847–1931)](../research/philosophy/logic-and-math/formal-revolution.md) — In 84 years, logic went from a philosophical discipline to the foundation of mathematics, computing, and the limits of knowledge itself.
- [The GDP-Identity Tariff Fallacy](../research/economics/gdp-identity-tariff-fallacy.md) — The popular argument "tariffs raise GDP because they close the trade gap (GDP = C+I+G+X−M, so reducing M raises GDP)" treats an accounting identity as a behavioral model.
- [The Is-Ought Problem and Performative Grounding](../research/philosophy/morality/is-ought-and-performative-grounding.md) — The is-ought gap dissolves — not through logical bridging, but because the questioner is already standing on the other side.
- [The Translation Problem](../research/philosophy/logic-and-math/the-translation-problem.md) — Formal logic is only as good as the translation from reality into symbols.
- [The Weighting Problem](../research/philosophy/epistemology/weighting-problem.md) — Objective measurements do not produce objective composite judgments.
- [The Regress Problem](../research/philosophy/epistemology/the-regress-problem.md) — Agrippa's trilemma: every justification bottoms out in brute fact, regress, or loop — all unjustified first principles — and the two ways past it.
- [The Map of Philosophy](../research/philosophy/the-map-of-philosophy.md) — the textbook map of the four domains bridged to the vault's Four Trunks.
- [Objectivism vs. Libertarian Voluntarism (Leibowitz vs Lloyd)](../research/debates/objectivism-libertarianism-leibowitz-lloyd.md) — the objective/subjective deadlock as a missing-Category-3 specimen; Lloyd's omniscience bar as the Agrippa over-reach.
- [The Three-Layer Method — Spec, Verifier, Environment](../research/karpathy-three-layer-method.md) — the robot-librarian model (doesn't know which book it's missing); verification as the only real lever; "outsource thinking, not understanding"
- [Sitch vs. Ryan Mullally — Birthright Citizenship & Natural Law](../research/debates/sitch-mullally-birthright-citizenship.md) — re-derives constitutive realism from a cold start; durability=detector vs structure=definition (verifier≠verified); "one should think" as candidate absolute + the universality↔implementability curve
- [Combinatorial vs Generative Design Space](../research/gaming/combinatorial-vs-generative-design-space.md)
- [Constructed ≠ Arbitrary](../research/philosophy/constructed-not-arbitrary.md) — *functional failure* as a third diagnostic beside convergence and collapse: the continuous, present-tense read of the signal collapse only delivers retrospectively; plus the *variance-at-the-margin* rule against low-N cherry-picking
- [Is Masculinity in Crisis? (Word War — Owlish vs. Garcia)](../research/debates/wordwar-masculinity-crisis-owlish-garcia.md) — a definition that immunizes its subject against all evidence; and the gender paradox quoted in the half that favors the speaker (attempts skew female, deaths male)
- [Is Feminism Helping Modern Relationships? (Word War — Ruelas vs. Anton)](../research/debates/wordwar-feminism-relationships-ruelas-anton.md) — four diagnostics for reading outcome statistics: stated reason ≠ cause (a survey category is not a mechanism), specify the window before accepting a rate (transition effects), scope the beneficiary, and federated legal questions are underspecified without jurisdiction *and* date
- [Is AI Making People Dumber? (Word War — Charsky vs. Bunn)](../research/debates/wordwar-ai-making-people-dumber-charsky-bunn.md) — cognitive offloading as *reshuffling* vs. atrophy; the vault's line neither debater states: **outsource computation, never verification** (if you can't verify, you can't detect when the tool is wrong)
- [AI as a Cognitive Tool](../research/ai-as-cognitive-tool.md) — the verification line: a tool you cannot check is a tool whose failures you cannot see; a child offloading before the capacity forms has nothing to verify *with*
- [Reading Outcome Statistics](../research/philosophy/epistemology/reading-outcome-statistics.md) — four ways a true number yields a false conclusion (jurisdiction+date, specify the window, stated reason ≠ cause, scope the beneficiary); they stack
- [Variance Is Not Luck](../research/economics/variance-is-not-luck.md) — a predicate true of every case carries no information about any case: the *if everything is luck, nothing is luck* reductio establishes **vacuity, not falsehood** — and vacuity is fatal when the term is supposed to do explanatory work. Same instrument as the differential test in [Generational Attribution](../research/philosophy/dynamics/generational-attribution.md)
- [Is the Algorithm Fair? (Word War — Cruz vs. Hamm)](../research/debates/wordwar-algorithm-fair-cruz-hamm.md) — **ranking is a compression problem**: alphabetical → categories → search → margin-weighted, with no neutral rung. Any ordering of a set too large to inspect requires a criterion, and a criterion encodes priorities — so "unbiased ranking" is not being withheld, it does not exist. A technical-domain specimen of [The Weighting Problem](../research/philosophy/epistemology/weighting-problem.md)
- [The Load-Bearing Word](../research/debates/the-load-bearing-word.md) — what makes a claim checkable: a standard with no failure condition is not a standard, an evidential claim ("X is a sign of Y") must vary with what it signals, and a modal claim over an outcome that happens regardless has no room to range
- [Absolutes and Differentials — Frame Selection as the Political Crux](../research/philosophy/epistemology/absolutes-and-differentials.md) — hub — political disputes settled by *frame selection* before evidence: absolute (level ⇒ a task) vs. differential (gap ⇒ an enemy); plus one-legible-criterion-for-a-multivariate-reason (WMD / "Iran cannot build a bomb"). Parent = the Weighting Problem's selection step.
- [Generational Attribution](../research/philosophy/dynamics/generational-attribution.md) — A cohort large enough to be pivotal in every vote will correlate with every outcome.
- [Easy to Critique, Hard to Build](../research/philosophy/tangents/easy-to-critique-hard-to-build.md) — costs are forecastable, benefits are not; decision rights follow the residual
- [Formal Debate Concepts — A Portable Inventory](../research/debates/formal-debate-concepts.md) — fiat, topicality, inherency, kritik, turns; analysis not prediction
- [Abortion Should Be Legal Nationwide (Word War R2 — Kewl Vic vs. Octavius Thunder)](../research/debates/wordwar-r2-abortion-vic-thunder.md)
- [Is the Iran War Justified? (Word War R3 — Tareyak vs. Octavius Thunder)](../research/debates/wordwar-r3-iran-justified-tareyak-thunder.md)
- [War With Iran: Would It Benefit America? (Word War R3 — Tejeda vs. Rex Jones)](../research/debates/wordwar-r3-iran-beneficial-tejeda-jones.md)
- [Flock Cameras: Beneficial or Harmful? (Word War SF2 — Summerhays vs. Rex Jones)](../research/debates/wordwar-sf2-flock-cameras-summerhays-jones.md)
- [All Drugs Should Be Legal (Word War SF1 — Thunder vs. Medeiros)](../research/debates/wordwar-sf1-drug-legalization-thunder-medeiros.md)
- [The Slack in the Law](../research/philosophy/morality/legal-theory/slack-in-the-law.md)
- [Common Law as a Discovery Procedure](../research/philosophy/morality/legal-theory/common-law-as-discovery.md)
- [The Absoluteness Claim](../research/philosophy/logic-and-math/the-absoluteness-claim.md)
- [Formal Systems and Approximation](../research/philosophy/logic-and-math/formal-systems-and-approximation.md)
- [The Consistency Defeater](../research/philosophy/logic-and-math/the-consistency-defeater.md)
- [The SQL Argument](../research/philosophy/logic-and-math/the-sql-argument.md)
- [Theorems With Absurd Readings](../research/philosophy/logic-and-math/theorems-with-absurd-readings.md)
- [Logics In Production](../research/philosophy/logic-and-math/logics-in-production.md)
- [Malpass vs Dyer — TAG and the Laws of Logic](../research/debates/malpass-dyer-tag-logic.md)
- [The Parity Move](../research/philosophy/epistemology/the-parity-move.md)
